...
class ZPAClient:
    ...
    def __init__(self):
        ...
        if not all([self.client_id, self.client_secret, self.identity_base_url, self.customer_id]):
        ...
        logger.debug(f"ZPAClient initialized with identity URL: {self.identity_base_url}")
    ...
    def authenticate(self) -> None:
        ...
        logger.debug(f"Client ID: {self.client_id[:10]}..." if self.client_id else "Client ID not set")
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if response.status_code == 400:
                    logger.error("400 Bad Request - Check credentials and IDENTITY_BASE_URL")
                    logger.debug("Common causes:")
                    logger.debug("  1. Invalid client_id or client_secret")
                    logger.debug("  2. Wrong IDENTITY_BASE_URL (should be like https://yourcompany.zslogin.net)")
                    logger.debug("  3. Credentials not properly configured for OAuth 2.0")
                elif response.status_code == 401:
                    logger.error("401 Unauthorized - Check if credentials are valid and active")
            ...
            logger.debug(f"Token (first 20 chars): {self.access_token[:20]}..." if self.access_token else "No token")
    ...
    def update_application_segment(self, segment_id: str, segment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully updated segment: {segment_data.get('name', segment_id)}")
...
def compare_segments(local: Dict[str, Any], remote: Dict[str, Any]) -> Dict[str, Any]:
    ...
    compare_keys = [
        'name', 'description', 'enabled', 'domainNames', 'serverGroups',
        'tcpPortRanges', 'udpPortRanges', 'icmpAccessType', 'ipAnchored',
        'healthReporting', 'bypassType', 'configSpace', 'defaultIdleTimeout',
        'defaultMaxAge', 'sraAppsDto', 'tcpKeepAlive'
    ]
...
def preview_diff(client: ZPAClient) -> None:
    ...
    try:
        ...
        total_changes = len(added_segments) + len(removed_segments) + len(modified_segments)
        ...
        if modified_segments:
            ...
            for seg in modified_segments:
                ...
                if len(fields_changed) <= 3:
                    ...
                    print(f"    Changed fields: {', '.join(fields_changed[:3])}, and {len(fields_changed)-3} more...")
    ...
    for i, segment in enumerate(changed_segments):
        ...
        if apply_all:
            ...
            if response == 'q':
                ...
                confirm_all = input("Type 'yes' to confirm, anything else to cancel: ").strip().lower()
                if confirm_all == 'yes':
                    apply_all = True
                    apply_change = True
                    print("Applying all remaining changes...")
                else:
                    print("Cancelled 'apply all'. Continuing with individual confirmations.")
                    apply_change = False
            elif response == 'y':
                apply_change = True
            else:  # 'n'
        ...
        if apply_change:
            try:
                ...
                for field in ['id', 'creationTime', 'modifiedTime', 'modifiedBy', 'createdBy']:
                ...
                if apply_all:
                    cont = input("\nError occurred. Continue with remaining updates? (y/n): ").lower()
                    if cont != 'y':
...
def search_segments_by_name(segments: List[Dict[str, Any]], search_str: str) -> List[Dict[str, Any]]:
...
def manage_server_groups_local():
    ...
    try:
        ...
        for i, sg in enumerate(server_groups, 1):
            print(f"{i}. {sg['name']} (ID: {sg['id']}) - Used in {sg['count']} segments")
        
        while True:
            try:
                choice = input(f"\nSelect server group (1-{len(server_groups)}) or 'q' to cancel: ").strip()
                if choice.lower() == 'q':
                    return
        ...
        # Search for segments
        search_str = input("\nEnter search string for segment names (or press Enter for all): ").strip()
        ...
        while True:
            ...
            for i, segment in enumerate(matching_segments):
                ...
                status = "[Has this group: Yes]" if has_group else "[Has this group: No]"
                
                print(f"{prefix} {checkbox} {segment.get('name', 'Unknown'):40} {status}")
        ...
        if changes_count == 0:
            print("No changes needed - selected segments already have the correct state.")
            ...
            for orig_segment in segments:
                if orig_segment['id'] == segment['id']:
                    if operation == 'a':
                        ...
                        if not segment_has_server_group(orig_segment, selected_sg['id']):
                        ...
                        if 'serverGroups' in orig_segment:
                            orig_segment['serverGroups'] = [
                                sg for sg in orig_segment['serverGroups']
                                if not (isinstance(sg, dict) and sg.get('id') == selected_sg['id'])
                            ]
                    break
...
def analyze_segments():
    ...
    try:
        ...
        segments_by_domain_count = sorted(
            segments, 
            key=lambda x: len(x.get('domainNames', [])), 
            reverse=True
        )[:5]
...
def main():
    ...
    try:
        ...
        logger.debug("Full exception details:", exc_info=True)  # Only log traceback in debug mode
        sys.exit(1)
